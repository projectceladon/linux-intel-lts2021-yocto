...
long ptp_ioctl(struct posix_clock_context *pccontext, unsigned int cmd, unsigned long arg)
{
...
__poll_t ptp_poll(struct posix_clock_context *pccontext, struct file *fp, poll_table *wait)
{
	struct ptp_clock *ptp = container_of(pccontext->clk, struct ptp_clock, clock);
